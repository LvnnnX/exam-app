...
DO $$ 
BEGIN
  -- 0. ADD QUESTION_IDS TO PLAYER
  IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name='player' AND column_name='question_ids') THEN
    ALTER TABLE player ADD COLUMN question_ids INT[];
  END IF;
