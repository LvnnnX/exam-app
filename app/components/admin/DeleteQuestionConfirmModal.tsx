...
export default function DeleteQuestionConfirmModal({
  ...
  return (
    <AnimatePresence>
      {deletingQuestion && (
        <motion.div
          className="fixed inset-0 bg-black/30 backdrop-blur-2xl flex items-center justify-center p-4 z-[9999]"
          initial={{ opacity: 0 }}
          animate={{ opacity: 1 }}
          exit={{ opacity: 0 }}
        >
          <motion.div
            className={`rounded-[24px] shadow-ios-xl max-w-md w-full p-6 ${theme === 'dark' ? 'bg-dark-800' : 'bg-white'}`}
